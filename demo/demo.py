...
def position(test_images_dir,filename):
    ...
    def demo_posiotion(sess, net, image_name):
        ...
        # Load the demo image
        im_file = os.path.join(cfg.FLAGS2["data_dir"], 'test_images', image_name)
...
def  cut(filename):
    ...
    def vis_detections(im, class_name, dets, thresh=0.5):
        ...
        for i in inds:
            ...
            if score > 0.97:
                ...
                img = cv_imread("test_result/position/" + filename)
                cropped = img[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]  # 裁剪坐标为[y0:y1, x0:x1]
                targetPath = r'test_result/cut/' + filename.split('.')[0]
    ...
    args = parse_args()
    demonet = args.demo_net
    tfmodel = r'lib/models/model_cut/vgg16_faster_rcnn_iter_8000.ckpt'
    # 路径异常提醒
    if not os.path.isfile(tfmodel + '.meta'):
        raise IOError(('{:s} not found.\nDid you download the proper networks from '
                       'our server and place them properly?').format(tfmodel + '.meta'))
...
def distinguish(filename,result_txt,test_images_dir):
    import os
    import tensorflow as tf
    from PIL import Image
...
if __name__ == '__main__':
    ...
    filelist = []
    rootdir = r"test_result/"# 选取删除文件夹的路径,最终结果删除img文件夹
    ...
    for filename in os.listdir(test_images_dir):
        ...
        converet(filename)
        tf.reset_default_graph()#重置tensorflow的旧变量，重置图标
        distinguish(filename,result_txt,test_images_dir)
        tf.reset_default_graph()#重置tensorflow的旧变量，重置图标
