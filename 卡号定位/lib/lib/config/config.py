...
tf.app.flags.DEFINE_integer('batch_size', 100, "Network batch size during training")
tf.app.flags.DEFINE_integer('max_iters', 8000, "Max iteration")
tf.app.flags.DEFINE_integer('step_size',100, "Step size for reducing the learning rate, currently only support one step")
#显示频数
tf.app.flags.DEFINE_integer('display', 1, "Iteration intervals for showing the loss during training, on command line interface")
...
#保存
tf.app.flags.DEFINE_integer('snapshot_iterations', 8000, "Iteration to take snapshot")
...
FLAGS2["data_dir"] = osp.abspath(osp.join(FLAGS2["root_dir"], ''))
